...
const getConnections = function(trip, blockedUsers) {
  let filteredUsers = Object.keys(blockedUsers)
    .concat(trip.details.trip_user)
    .map(user => `CAST(${user} AS varchar)`)
    .join(",");
  return knex
    .raw(
      `
...
const getAllChatsForUser = async function(auth_id) {
  ...
  return knex
    .select("chat_id", "user1", "user2", "updated_at")
    .from("chats")
    .then(allChats => {
      let myChats = allChats.filter(
        chat =>
          Object.values(chat).includes(auth_id) &&
          !(chat.user1 in blockedForUser || chat.user2 in blockedForUser)
      );
      let sortedChats = myChats.sort(
...
const getAllTripsForUser = async function(auth_id) {
  ...
  return knex
    ...
    .then(userTrips => {
      ...
      return Promise.all(
        userTripsArray.map(userTrip => getConnections(userTrip, blockedForUser))
      );
    });
};
...
blockUser = blockInfo => {
  ...
  let blockQueries = [
    knex("users")
      .where({ auth_id: blockInfo.user })
      .update({ blocked: blockInfo.newUserBlocked })
      .then(data => {
        let updateQueries = [
          getAllChatsForUser(blockInfo.user),
          getAllTripsForUser(blockInfo.user),
          knex.raw(`SELECT * FROM users WHERE auth_id = :auth_id`, {
            auth_id: blockInfo.user
          })
        ];
        return Promise.all(updateQueries);
      }),
    knex("users")
      .where({ auth_id: blockInfo.toBlock })
      .update({ blocked: blockInfo.newOtheruserBlocked })
  ];
  return Promise.all(blockQueries).then(updates => {
    return {
  ...
  return knex("trips")
    ...
    .then(createdTrip => {
      return getConnections(
        { details: createdTrip[0], connections: [] },
        blockedForUser
      );
    });
};
...
updateChat = update => {
  ...
  if (update.action === "new message") {
    return knex("chats")
      .where({ chat_id: update.chat.chat_id })
      .update({
